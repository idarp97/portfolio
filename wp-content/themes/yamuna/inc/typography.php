...
if ( ! function_exists( 'yamuna_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'yamuna_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function yamuna_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', yamuna_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $yamuna_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $yamuna_settings[$key], ':' ) !== false ) {
					$yamuna_settings[$key] = current( explode( ':', $yamuna_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $yamuna_settings[$key] );

				// Grab the variants using the plain name
				$variants = yamuna_get_google_font_variants( $yamuna_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'yamuna_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'yamuna_fonts_subset','' );
		...
		$default_fonts = YAMUNA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('yamuna-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('yamuna-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'yamuna_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'yamuna_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function yamuna_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'yamuna_fonts_customize_register' ) ) {
			// Bail if Yamuna Premium is active
			return;
		}
		...
		$defaults = yamuna_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Yamuna_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'yamuna_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'yamuna_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'yamuna_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Yamuna_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'yamuna' ),
					'settings' => array(
						'family' => 'yamuna_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'yamuna_settings[body_font_weight]',
						'transform' => 'yamuna_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'yamuna_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Yamuna_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'yamuna' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'yamuna_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'yamuna_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'yamuna_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Yamuna_Range_Slider_Control(
				...
				array(
					'type' => 'yamuna-range-slider',
					'description' => __( 'Line height', 'yamuna' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'yamuna_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'yamuna_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Yamuna_Range_Slider_Control(
				...
				array(
					'type' => 'yamuna-range-slider',
					'description' => __( 'Paragraph margin', 'yamuna' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'yamuna_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'yamuna_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function yamuna_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'yamuna_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'yamuna_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'yamuna_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_yamuna_get_all_google_fonts_ajax', 'yamuna_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function yamuna_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'yamuna_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'yamuna_customize_nonce' ] ), 'yamuna_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'yamuna_customize_nonce', 'yamuna_customize_nonce' );
		...
		$fonts = yamuna_get_all_google_fonts();
...
if ( ! function_exists( 'yamuna_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: yamuna_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function yamuna_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, yamuna_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'yamuna_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'yamuna_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: yamuna_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function yamuna_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'yamuna_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function yamuna_get_font_family_css( $font, $settings, $default ) {
		$yamuna_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'yamuna_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $yamuna_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'yamuna_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . yamuna_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'yamuna_add_to_font_customizer_list' ) ) {
	add_filter( 'yamuna_typography_customize_list', 'yamuna_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function yamuna_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = yamuna_get_all_google_fonts();
		$select_fonts = yamuna_get_all_google_fonts( apply_filters( 'yamuna_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $yamuna_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $yamuna_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $yamuna_settings[ $setting ], yamuna_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $yamuna_settings[ $setting ] ) ) ] = array(
				'name' => $yamuna_settings[ $setting ],
		...
		if ( function_exists( 'yamuna_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'yamuna_secondary_nav_settings', array() ),
				yamuna_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], yamuna_typography_default_fonts() ) ) {
