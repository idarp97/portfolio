...
add_action( 'after_setup_theme', 'yamuna_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function yamuna_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'yamuna_construct_sidebars' );
}

if ( ! function_exists( 'yamuna_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'yamuna_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function yamuna_woocommerce_start() { ?>
...
if ( ! function_exists( 'yamuna_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'yamuna_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function yamuna_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'yamuna_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'yamuna_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function yamuna_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'yamuna_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'yamuna_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'yamuna_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function yamuna_bbpress_css() {
...
if ( ! function_exists( 'yamuna_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'yamuna_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function yamuna_buddypress_css() {
...
if ( ! function_exists( 'yamuna_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'yamuna_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function yamuna_beaver_builder_css() {
