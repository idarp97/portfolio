( function( api ) {
	...
	api.controlConstructor['yamuna-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.yamuna-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the yamuna_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'yamuna_get_all_google_fonts_ajax',
								yamuna_customize_nonce: yamuna_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.yamuna-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.yamuna-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.yamuna-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.yamuna-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.yamuna-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.yamuna-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.yamuna-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.yamuna-font-variant' );
			...
			control.container.on( 'change', '.yamuna-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.yamuna-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.yamuna-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.yamuna-font-family select' ).selectWoo();
	jQuery( '.yamuna-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".yamuna-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.yamuna-font-variant' ).hide();
		}
	});
