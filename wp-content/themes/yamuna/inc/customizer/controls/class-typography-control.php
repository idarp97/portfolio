...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Yamuna_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Yamuna_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'yamuna-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'yamuna-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), YAMUNA_VERSION, true );
			wp_enqueue_style( 'yamuna-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), YAMUNA_VERSION );

			wp_enqueue_script( 'yamuna-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'yamuna-typography-selectWoo' ), YAMUNA_VERSION, true );
			wp_enqueue_style( 'yamuna-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), YAMUNA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'yamuna_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'yamuna' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'yamuna' );
			$this->json[ 'google_fonts' ] = apply_filters( 'yamuna_typography_customize_list', yamuna_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = yamuna_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'yamuna' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'yamuna' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'yamuna' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'yamuna' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="yamuna-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="yamuna-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="yamuna-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="yamuna-font-transform">
					<label>
